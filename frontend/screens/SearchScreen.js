...
const SearchScreen = () => {
  ...
  const handleSearch =async() => {
    ...
    try {
      const response = await fetch(`http://172.31.93.14:8080/api/v1/fetch/search?searchItem=${searchKey}`); // Replace with your API endpoint
      if (!response.ok) {
      ...
      if(jsonData.data.length===0) {Alert.alert("No Data Found")};
